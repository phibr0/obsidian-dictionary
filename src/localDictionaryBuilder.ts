...
export default class LocalDictionaryBuilder {
    ...
    async newNote(content: DictionaryWord): Promise<void> {
        ...
        let file: TFile;
        const langString = settings.defaultLanguage.split("(").last().replace(")", "");

        try {
            file = await plugin.app.vault.create(`${settings.folder ? settings.folder + '/' : ''}${settings.prefix.replace(/{{lang}}/ig, langString)}${settings.capitalizedFileName ? this.cap(content.word) : content.word}${settings.suffix.replace(/{{lang}}/ig, langString)}.md`,
                settings.template
